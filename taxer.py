...
class TaxerSession:
  ...
  def __init__(self, *, auth_data, basename=None, extra_headers=None, init_session=True):
    """
    :param basename no trailing slashes!
    """
    self.logger = logging.Logger(f'{self.__class__.__module__}.{self.__class__.__name__}')
  ...
  def request(self, method, path, body, params=None):
    assert self._session is not None, "You must call init_session explicitly," \
                                      " if skippied upon initialization"

    if params is None:
      params = {'lang': 'ru'}

    self._last_response = r = self._session.request(
      method, self.make_url(path), headers=self._common_headers,
      data=json.dumps(body, cls=CustomJSONEncoder), params=params,
    )
    formatted = r.json()
...
class TaxerAPI:
  ...
  def make_timestamp(self, time: datetime):
    """
  ...
  def get_operations(self, date_to):
    """
    """
    return self.session.request('get', 'finances/operation/load', {
  ...
  def convert_iban_to_taxer_acc(self, account_number, response='for_operation'):
    if getattr(self, '_accounts_cache', None) is None:
      self._account_cache = self.get_accounts()['accounts']
    
    for acc in self._account_cache:
      if account_number == acc['num']:
        if response == 'for_operation':
          return {'id': acc['id'], 'currency': acc['currency'], 'title': acc['title']}
        elif response == 'full':
          return acc
    
    raise UnknownAccountNumber(f'Account number is not in the system {account_number}')
  ...
  def add_operation(
    ...
    return self.session.request(
      'post', 'finances/operation/create',
      payload,
    )
  ...
  def add_exchange(self,
                   ...
                   account_from: str, account_to: str, time: datetime):
    """
